...
class ClassicalCompressorUltimate:
    """古风压缩器 - 极致无典故版本，追求最高token节省"""
    ...
    def _apply_ultimate_optimization(self, text: str) -> str:
        ...
        # 用→替换连接词
        result = re.sub(r'(因为|由于|所以|因此|于是|导致|引起|造成|使得|致使)', '→', result)
    ...
    def test_compression(self):
        ...
        test_cases = [
            ...
            # 更复杂的例子
            "您好，请问如何解决数据库连接超时的问题？",
            "感谢您的帮助，我已经尝试了重启服务器，但是问题依然存在。",
            "不好意思，我想请教一下关于React性能优化的问题。",
            "麻烦您帮我检查一下网络配置是否正确。",
        ]
        
        print("=" * 70)
        print("古风小生模式 - 极致无典故压缩测试")
        print("=" * 70)
        ...
        for i, test in enumerate(test_cases, 1):
            ...
            print(f"字统计: {analysis['original_length']}字 → {analysis['compressed_length']}字")
            print(f"字符统计: {analysis['original_chars']}字符 → {analysis['compressed_chars']}字符")
        ...
        # 总体统计
        char_reduction = (total_original_chars - total_compressed_chars) / total_original_chars * 100
        print(f"\n" + "=" * 70)
        print(f"总体统计:")
        ...
        print(f"总体节省: {round(char_reduction, 1)}% ({total_original_chars - total_compressed_chars}字符)")
        print("=" * 70)
        ...
        return {
            ...
            "char_reduction": round(char_reduction, 1)
        }


if __name__ == "__main__":
    compressor = ClassicalCompressorUltimate()
    ...
    print("4. 用→符号代替文字连接")
    print("5. 标点符号简化")
    print(f"\n字符节省: {results['char_reduction']}%")
    print(f"平均每100字节省约{int(results['char_reduction'])}字")